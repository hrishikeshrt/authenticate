...
doCommands() {
  ...
  echo "Running ironport authenticate .."

  # Check config status
  CONFIG="$HOME/.iitk-config"
  [ -f $CONFIG ] || CONFIG="/usr/share/iitk-auth/config"
  [ -f $CONFIG ] || (log 1 "No config file found." && stopDaemon)

  export user="`sed -n '1 p' ${CONFIG}`"
  export pass="`sed -n '2 p' ${CONFIG}`"
  export ip="`sed -n '3 p' ${CONFIG}`"

  ([ -z "$user" ] || [ -z "$pass" ] || [ -z "$ip" ]) &&  (log 1 "Invalid config." && stopDaemon)
  
  refurl='http://authenticate.iitk.ac.in/netaccess/connstatus.html'
  authurl='http://authenticate.iitk.ac.in/netaccess/loginuser.html'
  authurl1='https://ironport1.iitk.ac.in/B0001D0000N0000N0000F0000S0000R0004/'${ip}'/http://www.google.com/'
  authurl2='https://ironport2.iitk.ac.in/B0001D0000N0000N0000F0000S0000R0004/'${ip}'/http://www.google.com/'

  [ -f "/tmp/auth" ] || rm "/tmp/auth"
  [ -f "/tmp/auth1" ] || rm "/tmp/auth1"
  [ -f "/tmp/auth2" ] || rm "/tmp/auth2"
...
setupDaemon() {
  ...
  if [ ! -f "$logFile" ]; then
    ...
    size=$((`ls -l "$logFile" | cut -d " " -f 5`/$logMaxSize))
...
loop() {
  ...
  if [ $((now-last+runInterval+1)) -lt $((runInterval)) ]; then
    ...
    log "Sleep $((now-last+runInterval))."
  fi
...
log() {
  ...
  "1")
  shift
  logger -p daemon.err -st IronPort "$@"
  ;;
  *)
  logger -p daemon.notice -t IronPort "$@"
  ;;
  esac
